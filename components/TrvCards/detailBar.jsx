    ...
    <div className="db shadow-md w-[100%] h-[18%] bg-off-white flex rounded-medium text-gray-700 hover:bg-gray-100">
      ...
      <div className="w-[93%] bg-transparent border-2 border-peela rounded-l-large flex">
        ...
        <div className="flex translate-x-[-4%] mt-[10.5vh] z-10 m-auto" style={{zIndex:"10"}}>
          ...
          { props.link && <a href = {url} target="_blank"><button className="bg-peela hover:duration-100 hover:bg-halka-peela ml-2 rounded-md duration-250 w-auto text-[1rem] px-1">View</button></a> }
          { !props.link && 
              // <UploadButton id = {props.id} email = {props.email} from = {props.w} userid = {props.userid} onClick={props.handleUpload}/>
              <div className="inline-block rounded-md ml-2 duration-250 text-sm text-center bg-peela text-gray-700 cursor-pointer hover:bg-amber-400 px-1" onClick={props.handleUpload}>Upload</div>
          }
        </div>
